...
def _ensure_cuda_runtime_on_path() -> None:
    ...
    en tiempo de carga. En vez de rastrear el disco (lento y poco confiable
    en un arbol tan grande como /usr en Colab), le preguntamos directamente
    al paquete pip nvidia-cuda-runtime-cu12 donde se instalo: es instantaneo.
    """
    lib_dir = None
    try:
        import nvidia.cuda_runtime
        pkg_dir = Path(list(nvidia.cuda_runtime.__path__)[0])
        candidate = pkg_dir / "lib"
        if candidate.exists():
            lib_dir = candidate
    ...
    if lib_dir:
        ...
        print(
            "Aviso: no se encontro el paquete nvidia-cuda-runtime-cu12. "
            "Instalarlo con: !pip install -q nvidia-cuda-runtime-cu12"
        )
