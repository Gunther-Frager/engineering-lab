...
def _ensure_cuda_runtime_on_path() -> None:
    ...
    en tiempo de carga. Colab la trae via el paquete pip nvidia-cuda-runtime-cu12
    pero no la deja en el LD_LIBRARY_PATH por defecto. La agregamos antes de
    importar llama_cpp, que es lo primero que la busca.
    """
    candidates = glob.glob(
        "/usr/local/lib/python3.*/dist-packages/nvidia/cuda_runtime/lib"
    )
    if candidates:
        current = os.environ.get("LD_LIBRARY_PATH", "")
        os.environ["LD_LIBRARY_PATH"] = ":".join(candidates + [current])
