...
def _ensure_cuda_runtime_on_path() -> None:
    """
    El wheel precompilado de llama-cpp-python (cu121) necesita libcudart.so.12
    en tiempo de carga. Su ubicacion real varia segun el entorno (Colab la
    instala en distintas rutas segun el momento), asi que en vez de asumir
    un path fijo, la buscamos de verdad antes de importar llama_cpp.
    """
    try:
        result = subprocess.run(
            ["find", "/usr", "/opt", "/root", "-name", "libcudart.so.12*"],
            capture_output=True, text=True, timeout=60,
        )
        found_dirs = {
            str(Path(line).parent)
    ...
    if found_dirs:
        ...
        os.environ["LD_LIBRARY_PATH"] = ":".join(list(found_dirs) + [current])
    else:
        print(
            "Aviso: no se encontro libcudart.so.12 en el sistema. "
            "Si falla la carga del modelo, correr en una celda: "
